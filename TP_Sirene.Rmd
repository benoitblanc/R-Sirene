...
```{r}
typeonecommerce <- filter(alim42, codeCommuneEtablissement %in% onlyonecommerce$codeCommuneEtablissement) %>%
  group_by(Type) %>%
...
```{r}
morethan100commerces <- group_by(data42, codeCommuneEtablissement, libelleCommuneEtablissement, Type) %>%
  summarise(nb_commerces=n()) #%>%
  #filter(nb_commerces>=100)
```