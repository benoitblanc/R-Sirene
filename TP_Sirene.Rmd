...
```{r, include=TRUE, echo=FALSE, out.width="300pt"}
...
prop_alimRA_ap1970 + guides(fill = "none") + geom_area(show.legend = FALSE) + facet_wrap(.~TypeAbreg, ncol=1,  scales = "fixed", labeller = labeller(TypeAbreg=labels)) + labs(x="Année", y="Taux d'entreprise") + theme_bw()
```
...
map(departements, show_commerces_dept)
```
...
RA_shp2 <- RA_shp %>% group_by(insee_comm) %>%
...
```{r echo=FALSE}
map(departements, show_nb_commerces_dept)
```
